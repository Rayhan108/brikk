...
import { useCreateAndUpdatePrivacyMutation, useGetPrivacyQuery, useGetTermsQuery } from "../../redux/feature/others/othersApi";
import toast from "react-hot-toast"; // Importing toast for notifications

const Privacy = () => {
  const [createPrivacy] = useCreateAndUpdatePrivacyMutation();
  const { data: terms, refetch, isLoading, isError, error } = useGetPrivacyQuery(undefined); // Fetch Privacy from the API
  ...
  useEffect(() => {
    if (terms?.data?.text) {
      setContent(terms.data.text); // Load from API response (from the database)
    } else {
  ...
  }, [terms]); // Re-run when `terms` data is updated from API
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await createPrivacy({ text: content }).unwrap();
    ...
    <div>
      <div className="flex justify-between font-title bg-[#2C3E50] px-3 py-2 rounded-md">
        <div className="flex justify-center items-center gap-5">
          <p className="text-[#ffffff] font-title text-3xl font-bold">Privacy Policy</p>
        </div>
      </div>
      <div className="container min-h-screen mt-16">
        <div className="mt-5 text-black">
          <JoditEditor
            ...
            config={config}
            tabIndex={1}
            onBlur={(newContent) => setContent(newContent)} // Update the content on blur
            onChange={() => {}} // On change handler (no action here)
          />
